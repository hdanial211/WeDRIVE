...
/* ═══════════════════════════════════════════════════════════════
   SEARCH POPUP MODULE (DeepSeek-style)
   Triggered by all [data-search-popup] or .search-field-compact inputs
   ═══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  var overlay, modal, spInput, spResults;
  var allCarsCache = [];
  ...
  function renderDefault() {
    ...
    spResults.innerHTML = '<div class="sp-section-label">All Cars</div>';
    renderList(allCarsCache.slice(0, 6), '');
  }
  ...
  function renderList(cars, q) {
    if (!cars.length) {
      spResults.innerHTML = [
        ...
        '  No cars found for <strong>"' + escHtml(q) + '"</strong>',
        '</div>'
      ].join('');
      return;
    }
    ...
    // Click to navigate to car details
    spResults.querySelectorAll('.sp-item').forEach(function (el) {
      el.addEventListener('click', function () {
        var id = el.getAttribute('data-car-id');
        var base = window.location.pathname.includes('/pages/') ? '' : 'customer/pages/';
        var detailPage = base
          ? base + 'car-details/car-details.html?id=' + id
          : '../car-details/car-details.html?id=' + id;
        closePopup();
  ...
  function resolveImg(car) {
    ...
    return prefix + 'shared/model/' + file;
  }
  ...
  function wireTriggers() {
    // All compact search fields on dashboard
    var compactInputs = document.querySelectorAll('.search-field-compact input');
    compactInputs.forEach(function (inp) {
    ...
    // Search button
    var searchBtn = document.querySelector('.search-btn-compact');
    if (searchBtn) {
    ...
    if (faqInput) {
      faqInput.addEventListener('focus', function () {
        openPopup(faqInput.value || '');
        faqInput.blur();
      });
    }

    // Any future [data-search-popup] elements
    var extras = document.querySelectorAll('[data-search-popup]');
