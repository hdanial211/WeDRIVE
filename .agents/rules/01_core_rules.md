...
8. **Wajib Logik Akal Operasi & Data Sebenar Sahaja (Strict Zero Fabricated Fields & Real Data Only)**: Sifar medan rekaan ("benda yang tak ada jangan diadakan" cth: odometer/perbatuan), sifar ulangan universal pada rekod individu ("benda yang semua pakai jangan tunjuk dekat individu" cth: HQ Melaka dan status sedia disewa), dan sifar hardcoded dummy data (Wajib real data pipeline via draf sesi/Supabase). Rujuk [04_navigation_and_ui.md](04_navigation_and_ui.md) & [06_code_and_backend.md](06_code_and_backend.md).
9. **Pantang Larang Mutlak Integriti Laporan (Strict Zero Touch Rule: REPORT FYP 3.docx)**: DILARANG SAMA SEKALI menyentuh, mengubah, menulis semula, menjalankan sebarang skrip automatik, atau memadam fail laporan tesis `REPORT/REPORT FYP 3.docx` (dan semua fail `.docx` laporan tesis pengguna). Fail ini adalah hak milik mutlak pengguna dan sebarang pengubahsuaian hanya dibenarkan atas arahan eksplisit pengguna sahaja.
